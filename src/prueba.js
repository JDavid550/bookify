...
const searchwithClick = (word,button)=>{
...
searchWithEnter();
searchwithClick(keyword,submit);
searchwithClick(keyword,submit);
...
const searchwithClick2 = (button,word)=>{
    button.addEventListener('click', ()=>{
        if (word.value.length>3) {
            executeSearch2(word);
        }
...
searchwithClick2(submitResponsive,keywordResponsive);
searchwithClick2(submit,keyword);

const executeSearch2 =(word)=>{
    if (articleItems.length>1){
        inactivate();
        pull2(word);   
    }else{    
